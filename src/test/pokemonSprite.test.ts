    ...
    function getMissingMasterlist(mlist, dirpath, excludes = []) {
        ...
        if (fs.existsSync(dirpath)) {
            ...
            for (const filename of files) {
                ...
                if (name.includes('_')) {
                    ...
                    if (ext !== 'json') {
                        ...
                        if (mlist.hasOwnProperty(id)) {
                            ...
                            if (!mlist.hasOwnProperty(id) && jsonFileExists) errors.push(`[${id}] missing key ${id} in masterlist for ${trimmedFilePath}`);
                ...
                } else if (!mlist.hasOwnProperty(name)) errors.push(`[${name}] - missing key ${name} in masterlist for ${trimmedFilePath}`);
                else {
